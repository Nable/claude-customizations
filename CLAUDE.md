...
- **Skills** — a `SKILL.md` with YAML frontmatter (`name`, `description`) under `skills/<skill-name>/`. The `description` is trigger text: it lists the phrases/keywords that should activate the skill, so it must be written for matching, not just documentation. None exist at the moment (the former appstore-pages skill was converted to the `/appstore-pages` command — deliberate one-shot generators work better as commands; reserve skills for knowledge Claude should apply opportunistically).
...
- **Cross-artifact consistency.** When a command emits paired files (e.g. `debts.md` + `debts.json`, `index.html` + `style-tokens.json`), they must share the same IDs and data. The generators form a pipeline — `/style-guide` → `/appstore-pages` (reuses `style-tokens.json`) → `/app-store-listing` (reads `web-pages/meta.json` for URLs/email) → `/marketing-advisor` (reads the listing) → `/press-kit` (reuses everything) — orchestrated end-to-end by `/publish`. When changing an artifact's schema or path, update every consumer in the chain.
