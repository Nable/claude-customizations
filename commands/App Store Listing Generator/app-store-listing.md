...
- **URL e contatti da `web-pages/`**: se esiste la cartella `web-pages/` (generata da `/appstore-pages`), leggi `web-pages/meta.json` — usa `email` come email di supporto e, se `domain` è valorizzato, compila Support URL (`<domain>/`) e Privacy Policy URL (`<domain>/privacy.html`). Se `domain` è `null`, chiedi all'utente il dominio di pubblicazione prima di ripiegare su `[DA COMPILARE]`.
